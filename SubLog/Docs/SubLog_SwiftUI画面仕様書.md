...
│     ├── 「今月の課金合計」
│     ├── thisMonthTotal（大きく）
│     └── monthDiff（前月より多い/少ない）
│
├── 今月の課金 Top3
│     └── thisMonthTop3（金・銀・銅バッジ + service.name + total）
│
└── アクティブなサブスク一覧（最大5件）
      ├── 各行：ServiceIconView / subscription.label / service.name / subscription.price
      └── 残日数バッジ（7日以内→黄 / それ以上→緑）

※ 月別棒グラフは `AnalyticsView` へ移動予定
```
...
| 操作 | 遷移先 |
|------|--------|
| サブスク行タップ | `SubscriptionDetailView(subscription:, service:)` |
| Top3行タップ | `serviceType` に応じた詳細画面 |
| 「すべて見る」タップ | `ActiveSubscriptionsView`（sheet） |

---

## Section 2.5：アクティブなサブスク一覧画面（ActiveSubscriptionsView）

### 画面の目的

アクティブなサブスクリプションだけを一覧で確認する。  
HomeView の「すべて見る」から sheet で表示する。
...
NavigationStack
└── List
      ├── 各行：ServiceIconView(size: 44)
      │        subscription.label
      │        service.name
      │        subscription.price
      │        RenewalBadgeView(daysUntilRenewal:)
      └── 行タップ → SubscriptionDetailView(subscription:, service:)
```
